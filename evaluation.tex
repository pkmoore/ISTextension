...
to store descriptive information about a file
that cannot be held by normal filesystem fields
due to their strictly defined and limited structure.
For example,
...
Failure to preserve timestamps has obvious implications
in records-keeping and archival situations
as well as any situation where
a chronological record of file manipulations is important.
Incorrect timestamps can cause odd behavior
in Makefiles, archival programs, and similar
software~\cite{NautilusTimestamps, SudoTimestamp}.
Given these issues, it is important to ensure
that time related metadata
(such as creation, modification, and access times)
are preserved when copying a file.
To evaluate identify failures on this front,
we used a checker that determines
whether the application applies
the appropriate timestamps to the destination file
by monitoring for system calls from the {\tt stat()} family,
which are used to retrieve the timestamps,
and system calls from the {\tt utime()} family,
which apply the retrieved timestamps to the destination file.
...
instead of reading from a potentially infinite device.
In our experience, applications that fail to perform this check
end up completely filling a disk or exhausting available memory --
...
Employing a more complex (than the NullMutator) mutator allows
CrashSimulator to inject anomalous situations into an application.
This forces applications
into an encounter with problematic situations
that elicit a response that we can evaluate.
To test
its capabilities
...
run forever when processing an ``infinitely long'' file.
Second,
it demonstrated that
simply reporting an application did or did not change its behavior
in the presence of an anomaly is insufficient
if CrashSimulator's user is unfamiliar what each result means.
Both of these issues are being corrected
by improving CrashSimulator's output
so that it more clearly describes
the nature of a given result
and why the user should be concerned.
