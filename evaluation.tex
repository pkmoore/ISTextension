...
categories discussed in Section~\ref{SEC:background}. \textbf{CHECK THIS} These bugs included
...
The second bug finding opportunity offered by AES is available after some
unusual condition has been injected into the running application.  To test
CrashSimulator's capabilities at this point we needed an environmental
anomaly that could be used against a wide variety of applications.  We
selected one that occurs
when an application running on a Linux system has to retrieve
