...
to ensure that the file being copied is not replaced between the time it is initially examined and the time it is opened for copying.
...
file and, later, apply them to the destination file.  Failure to do so can
        lead to security problems~\cite{AppleCodeSigning}.}
...
Our goal is to minimize these mistakes as incorrect reports can
undermine a developer's confidence in a tool.  In this section we discuss
situations where CrashSimulator made either false positive or false
negative reports.  False positive reports involve the tool reporting
a failure when the application actually handles an anomaly correctly.
False negative reports happen when the tool reports an application handles
...
\hline
    ...
    \caption{This table contains the times required to execute several coreutils applications under different conditions.
             ...
             CrashSimulator Replay time is the time required to execute it under CrashSimulator using the null mutator with no checker configured.
...
than the original program being executed without it.
