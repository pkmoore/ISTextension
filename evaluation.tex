...
Work using our prototype of CrashSimulator showed that
it is able to test most
applications that are able to tested with CrashSimulator
regardless of common testing impediments like programming language
differences or dependency issues.
The majority of cases where CrashSimulator failed to test
an application were because some previously-unsupported system call was
encountered.
The strategy of improving CrashSimulator by adding support for
system calls as they are encountered has been successful
as system calls used by an application is small and
static~\cite{Tsai:2016:SML:2901318.2901341}.  This process
has allowed us to improve our prototype to the point that it can support a
large number of popular, complex applications.

During the
course of this our user study participants were encouraged to report
unsupported system calls should they encounter
them while using the tool.
The wider exposure provided by our new users necessitated the addition of
ten new system calls.
Support
...
the effort of implementing support.
...
test or integration test, a CrashSimulator test can be run
against multiple
