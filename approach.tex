...
communications (sec.~\ref{SUBSUB:MonitoringAndManipulating}).
A checker will then determine whether the
application has responded correctly to
$X$(sec.~\ref{SUBSUB:AnalyzingResponse}).
...
In CrashSimulator, mutators work by receiving the sequence of system calls
an application makes and accepting situations where the sequence indicates
it is appropriate to simulate an anomaly.  It then uses its stored
description of its anomaly to carry out its simulation.  In the same vein,
we implemented checkers as finite automata that receive the sequence of
system calls made by the application.  Because checkers encode what a
correct response by the application should look like, they were constructed
to accept situations where they observed sequences indicative  of such a
response.  We discuss this decision making process in more detail in
section~\ref{SUBSUB:AnalyzingResponse}.
...
We then turned to implementing a way to monitor and manipulate an
...
easily identify opportunities to simulate anomalies.
