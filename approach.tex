...
If the checker doesn't accept, we report that it has not responded corrextly.
Once created,
...
thw behavior an application should undertake
...
the anomaly correctly, based on whether it observed the behavior for which it
was built to look.
...
Turning SEA into the concrete implementation that we named CrashSimulator
...
where we could influence an application’s interactions. Functioning on
a system call level allowed us to ignore low level details that could
hurt usability, while still capturing the type of environmental anomalies
we needed. The already robust tooling in the Linux kernel made the
interception and modification of system call results and side effects a
fairly simple process,  and the well-defined semantics in the Linux
system call streamlined implementation. Finally, our decision to operate
at this level would allow a tool to test applications written in any
language that can execute Linux system calls.

We also wanted any implementation of SEA to be repeatable, and to offer
an easy way to manipulate system calls. For this task, we employed a
modified {\tt rr} debugger~\cite{rrwebsite} to record and replay applications. The modifications permitted a {\tt strace}-style system call recording to be
output alongside {\tt rr's} normal recording format, creating a complete log of an application’s system call activity. We chose to make this
modification because
while {\tt rr}'s recording contains the system call information we need,
it is stored in an opaque format that changes frequently between versions.
...
Another challenge in implementing SEA was it required a method to simulate
anomalies. During testing and early applications we introduced a new
technique called {\it process set cloning}.  As noted above, the {\tt rr}
...
the original.  However, we extended this capability by liberating process
