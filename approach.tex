...
\begin{figure}[t]
  \center{}
  \fbox{\includegraphics[scale=.6]{images/statidentify}}
...
As a concrete example of the above step consider the measures {\tt curl} takes
to ensure it properly handles the data its user requests.  By default, curl
outputs the contents of the requested url to the terminal.  In cases where {\tt
stdout} to a file, curl instead outputs a progress bar to the terminal by
...
interfere with the data being retrieved from the specified url should both file
descriptors refer to the same file.

The checks that drive this flow are accomplished by using a system call from the
...
determine whether a file descriptor points to a data file (the file descriptor
refers to a regular file) or the terminal (the file descriptor refers to a
character device).  Well-behaved applications should make these checks and alter
...
checker with which other applications can be tested against the same set of
...
their results to simulate the desired combination.  See figure YYY for a diagram
of a potential underlying automaton for this mutator.  Some indication of the
