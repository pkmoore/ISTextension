...
Due to the fashion in which we chose to implement Results Based Simulation,
we are specifically concerned with issues from the above sources
that are visible in the results an side effects of the system calls
made by an application.
In our evaluation we take a deeper look at anomalies from
these categories in order to assess technique's effectiveness.
