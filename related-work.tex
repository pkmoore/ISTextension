...
applications, and testing protocols. This section
...
CrashSimulator. But, data mining techniques could be adapted in the years
ahead to automate this process.
...
and aid to program understanding~\cite{yuan2010sherlog,
...
system call traces for a different purpose: driving replay executions
in order to test responses to known environmental anomalies before an
application is deployed.  This more active approach can expose bugs that
are invisible to passive log mining.


\noindent
{\bf Environmental Influence and Cross-Platform Portability.}
Negotiating the influence of an environment into which an
application is deployed has been investigated from several
perspectives. One  is to build “portable” systems
...
system call interposition ~\cite{Guo:2011:CUS:2002181.2002202}.  This
technique can can also been used to detect and prevent security
...
mentioned in~\ref{rel-static-analysis}, have also been applied to
detection of differences between versions of external components.

CrashSimulator’s goals are complementary to these studies, however, our
tool kit is more focused on testing an application’s response to anomalies.
We are not interested in proving if the environments tested behave as
expected.
...
\preston{We need to cite: Zach Miller, Todd Tannenbaum, Ben Liblit:
Enforcing Murphy's Law for Advance Identification of Run-time Failures.
USENIX Annual Technical Conference 2012: 203-208. R. E. feedback from ISSTA
submission}
...
to utilize these methods, a knowledge of the nature of the failures needs
to be acquired, and the specific system properties must be specified.
...
applications other than the ones on which the anomalies were originally
discovered.
\fi
...
and so there may be deviations in results. Similarly, efficiently
solving a series of constraints in order to exercise a particular code
path can be can be difficult to guarantee that a particular set of
generated inputs will exercise the intended code path in many
circumstances due to external dependencies that the tool cannot
analyze. For example, a white-box testing tool cannot reliably generate
inputs that are guaranteed to exercise a code path that relies on an
operating system resource being available.  Finally, white-box tools
typically require that an application's source code be available which
is not always the case. Even advanced white-box tools that analyze an
application's machine code can be stymied in situations where an
...
tools do not need the capability to understand and analyze an
...
computationally inexpensive. The downside of simplicity is that they
...
terms of bug identification. Also, they cannot identify specifically
